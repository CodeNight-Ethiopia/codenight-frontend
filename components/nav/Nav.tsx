...
const Nav = () => {
	...
	return (
		<nav
			className={`sticky top-0 z-50 bg-white border-b border-gray-100 transition duration-300 ${
				fix ? "shadow-md" : ""
			}`}
		>
			<div
				className={`md:hidden absolute top-0 left-0 w-1/3 h-screen ${
					toggle ? "backdrop-blur-sm" : ""
				}`}
			/>
			...
			<div className="mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8">
				<div className="flex p-6 md:items-center items-start flex-row md:justify-between justify-center">
					...
					<div className="md:hidden flex flex-1 justify-end items-center ">
						...
						<div
							className={`p-4 bg-white absolute top-20 right-0 w-2/3 h-screen rounded-xl sidebar ${
								toggle ? "flex" : "hidden"
							}`}
						>
							<ul className="list-none flex flex-col gap-6 justify-start items-center flex-1 text-cyan-50 ">
								{NavigationMenuData.map((menu, idx) => (
									<NavItem key={menu.name} link={menu.link}>
										{menu.name}
									</NavItem>
